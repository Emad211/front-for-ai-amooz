...
# These four specifically discriminate the failure modes observed in pilot v2:
# GPT cross-image binding on Q65, visual/text fidelity on Q94/S57, and a Gemini
# source-formula misread on S133.
...
def _system_prompt() -> str:
    return (
        ...
        "for the named TARGET. The crop may include a thin strip from a neighboring item because "
        "of safety padding; ignore neighboring material with another printed question number. "
        "Check Persian words, digits, decimal marks, signs, units, answer labels, equations and "
        "LaTeX semantics, chemical formulae, omissions, hallucinations, reading order, diagrams, "
        "tables, and whether a source visual must be preserved. Equivalent whitespace/Markdown "
        "alone is not an error. A changed digit, operator, exponent, variable, chemical symbol, "
        "answer option, or meaningful omission is major or critical. Return ONLY one valid JSON "
        "object with no Markdown fence and no surrounding prose."
    )


def _json_contract() -> str:
    # Deliberately omit item_id. The request already binds the response to exactly one target,
    # so asking the model to echo an identifier creates unnecessary failure modes.
    return (
        ...
        "\"note\":\"short reason\"}]} . "
        "Do not add item_id or any other top-level key."
    )
...
def _parse_single_review(content: str, *, item_id: str) -> dict[str, Any]:
    # Deterministic local extraction tolerates harmless trailing braces/prose without any
    # extra provider call. Semantic validation remains strict.
...
def _cost_meta(root: Mapping[str, Any], *, status_code: int, latency_ms: float) -> dict[str, Any]:
...
def _review_once(
    *,
    ...
    prefix.with_suffix(".request.safe.json").write_text(
        json.dumps(
            {
    ...
    prefix.with_suffix(".provider.private.json").write_bytes(response.content)
    ...
    try:
        ...
        root = {}
    meta = _cost_meta(root, status_code=response.status_code, latency_ms=latency_ms)
    prefix.with_suffix(".response.safe.json").write_text(
    ...
    try:
        ...
        review = _parse_single_review(content, item_id=item_id)
...
class Command(BaseCommand):
    help = (
        "Calibrate OCR fidelity with one source crop per provider call on four discriminating "
        "regions. Uses two economical models, no retry, and no paid repair."
    )
    ...
    def handle(self, *args, **options):
        ...
        expected_config = _config(bundle=bundle)

        if output_dir.exists() and any(output_dir.iterdir()):
            if not resume:
                raise CommandError("Output directory is non-empty; use --resume only for this exact calibration run.")
            try:
                existing_config = json.loads((output_dir / "calibration-config.safe.json").read_text(encoding="utf-8"))
            except (OSError, ValueError, json.JSONDecodeError) as exc:
                raise CommandError("Resume directory has no valid calibration-config.safe.json.") from exc
        ...
        try:
            for model in _CALIBRATION_MODELS:
                for item in private_items:
                    ...
                    review_path = output_dir / f"verifier.{safe_model}.{current_item_id}.review.private.json"
                    if resume and review_path.is_file():
                        loaded = json.loads(review_path.read_text(encoding="utf-8"))
                        if not isinstance(loaded, Mapping) or loaded.get("itemId") != current_item_id:
                            raise RuntimeError("resume_review_invalid")
                        reviews_by_model[model].append(dict(loaded))
                        continue

                    prefix = output_dir / f"verifier.{safe_model}.{current_item_id}"
                    ...
                    call_row = {"model": model, "itemId": current_item_id, **meta}
                    call_meta.append(call_row)
            ...
            # Count provider responses from disk rather than only semantically accepted reviews.
            ...
            archive_path = shutil.make_archive(str(output_dir), "zip", root_dir=output_dir)
            raise CommandError(
                f"Single-item fidelity calibration failed; partial bundle={archive_path}"
            ) from exc
        ...
        provider_files = list(output_dir.glob("verifier.*.*.provider.private.json"))
        total_unit = sum(float(row.get("estimatedUnit") or 0) for row in call_meta)
        total_irt = sum(float(row.get("estimatedIrt") or 0) for row in call_meta)
        run_manifest = {
            "schemaVersion": 1,
            "privateDiagnosticBundle": True,
            "productionPipelineChanged": False,
            ...
            "echoedItemIdRequired": False,
            "estimatedTotalUnit": round(total_unit, 10),
            "estimatedTotalIrt": round(total_irt, 2),
            "acceptance": {
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                f"estimated_irt={run_manifest['estimatedTotalIrt']}, bundle={archive_path}"
            )
        )
