...
class TestStudentExamPrepChatMediaApi:
    ...
    def test_media_requires_file(self):
        ...
        resp = client.post(
            f'/api/classes/student/exam-preps/{session.id}/chat-media/',
            {'message': 'x'},
            format='multipart',
        )
    ...
    def test_image_upload_persists_user_and_assistant_messages(self, monkeypatch):
        monkeypatch.setattr('apps.classes.views.describe_exam_prep_handwriting', lambda **_kwargs: 'DESC')
        monkeypatch.setattr(
        ...
        resp = client.post(
            f'/api/classes/student/exam-preps/{session.id}/chat-media/',
            {'file': file, 'message': 'این چیه؟', 'question_id': 'q1'},
            format='multipart',
        )
    ...
    def test_audio_upload_persists_combined_voice_transcript(self, monkeypatch):
        ...
        resp = client.post(
            f'/api/classes/student/exam-preps/{session.id}/chat-media/',
            {'file': file, 'message': 'گفتم', 'question_id': 'q1'},
            format='multipart',
        )
        assert resp.status_code == 200
        assert resp.data['type'] == 'text'
