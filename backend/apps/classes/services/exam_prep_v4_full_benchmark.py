...
The harness owns no alternate extraction logic. Fake and live modes both use
private PDF preparation, classification persistence, exact Source Map
confirmation, the provider-neutral extraction runner, typed record persistence,
and deterministic matching. Only aggregate counts, timing, usage, and mismatch
...
class ManifestFakeExtractionProvider:
    """Deterministic fake outputs routed through real persistence and matching."""
    ...
    def detect_segment_blocks(self, *, document, segment, pages, images):
        ...
        if segment.role == ExamSourceRole.QUESTIONS:
            kind, numbers = 'question', self.question_numbers
        elif segment.role == ExamSourceRole.ANSWER_SOLUTIONS:
            kind, numbers = 'answer_solution', self.answer_numbers
        elif segment.role == ExamSourceRole.ANSWER_KEY:
            kind, numbers = 'answer_key', self.answer_numbers
        else:
            return {'blocks': []}
        return {
            'blocks': [
...
def _segment_mismatch_count(
    ...
    if expected == actual:
        return 0
    return max(len(expected), len(actual), 1)


@transaction.atomic
def _bind_canonical_source_map_fingerprint(
    document: ExamSourceDocument,
) -> str:
    locked = ExamSourceDocument.objects.select_for_update().get(id=document.id)
    pages = list(locked.pages.order_by('display_order', 'page_number'))
    if len(pages) != locked.page_count:
        raise FullBenchmarkError('Prepared Source Map is incomplete.')
    structural_map = structural_page_map_from_models(pages)
    canonical = source_map_fingerprint(
        structural_map,
        page_count=locked.page_count,
    )
    if locked.source_map_fingerprint != canonical:
        locked.source_map_fingerprint = canonical
        locked.save(update_fields=['source_map_fingerprint', 'updated_at'])
    ExamSourceSegment.objects.filter(
        document=locked,
        revision=locked.classification_revision,
    ).exclude(fingerprint=canonical).update(fingerprint=canonical)
    document.source_map_fingerprint = canonical
    return canonical
...
def _classify_and_confirm(
    *,
    ...
    mismatch_count = _segment_mismatch_count(fixture, classification)
    if mismatch_count:
        raise FullBenchmarkError(
            f'Fixture {fixture.fixture_id} source map has structural mismatches.'
        )
    canonical = _bind_canonical_source_map_fingerprint(document)
    confirm_teacher_source_map(
        teacher=teacher,
...
def _record_metrics(
    ...
    return {
        ...
        'missingExpectedOutOfScopeCount': len(
            expected_out_of_scope - actual_out_of_scope
        ),
...
def _usage_metrics(document_id: int, started_at: datetime) -> dict[str, Any]:
    ...
    for detail in (
        ...
        stage_rows = rows.filter(detail=detail)
        values = stage_rows.aggregate(
            inputTokens=Sum('input_tokens'),
            outputTokens=Sum('output_tokens'),
            totalTokens=Sum('total_tokens'),
            estimatedCostUsd=Sum('estimated_cost_usd'),
...
def run_full_pipeline_benchmark(
    *,
    ...
    try:
        ...
        totals: dict[str, int] = {
            key: sum(int(item[key]) for item in fixture_reports)
        ...
        report = {
            ...
            'models': {
                'classifier': (
                    classifier_model if mode == 'live_provider' else 'fake-provider'
                ),
                'block': block_model if mode == 'live_provider' else 'fake-provider',
                'question': (
                    question_model if mode == 'live_provider' else 'fake-provider'
                ),
