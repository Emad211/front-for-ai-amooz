...
class MistralSourceFirstAdapter:
    ...
    def _ensure_document(self, document: Any) -> tuple[OCR4DocumentResult, dict[str, Any]]:
        ...
        try:
            ...
            # A provider response can satisfy the transport/schema contract
            # and still contain a new block shape that the local deterministic
            # analyzer does not understand.  Treat that as derived-evidence
            # failure and let the V4 caller use its existing structured
            # detector; never turn an analyzer bug into a failed exam.
            error = SourceFirstError(
                "The OCR4 layout evidence could not be analyzed safely."
            )
            self._document_errors[key] = error
