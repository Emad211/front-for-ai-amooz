...
class AdminUserOrgManagerView(APIView):
    ...
    organization — the same identity the SMS onboarding flow produces. This lets
    an admin grant that status to an EXISTING user straight from the user panel,
    instead of only via the org-create + invite-code path.
