...
### Goal
Convert the source into STRICT, machine-readable JSON containing one ordered, flat list of questions — each captured VERY accurately.

### Critical rules (must follow)
- Keep the SAME language as the source. Do NOT translate.
- Do NOT invent questions, options, answers, or points.
...
- Preserve the source order of every question. Do not group or categorize questions into sections.
- Preserve math using LaTeX exactly as instructed below.
- The output must be valid JSON (no trailing commas, no comments).
...
### Important constraints
- Do not include any extra top-level keys.
- Always include the top-level `questions` list, even when no question can be extracted.
