...
/**
 * Cancel a running class-creation pipeline. The backend revokes the Celery
 * task and moves the session to the terminal `cancelled` status. Returns the
 * updated session detail (status === 'cancelled').
 */
...
/**
 * Cancel a running exam-prep pipeline. The backend revokes the Celery task and
 * moves the session to the terminal `cancelled` status.
 */
