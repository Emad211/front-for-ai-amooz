 ...
 * skeleton, no empty card — until it knows there are subjects to show. The
 * vast majority of students have no advisor, and a placeholder that reserved
 * space on every load would be a worse regression than this card is a
 * feature. A failed fetch is swallowed for the same reason. The endpoint is
 * already quiet on the wire (`active:false` for the no-advisor case, never a
 * 4xx), so the only thing left to guard is the render.
 */
