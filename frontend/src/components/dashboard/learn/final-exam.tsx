  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      initializeAssessment(
        q.exam_id,
        (q?.questions ?? []).map((question: FinalExamQuestion) => question.id),
      );
    } catch (e: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h3 className="text-base font-bold text-foreground"><MathText text={exam.exam_title} /></h3>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 sm:gap-3">
                  {[
                    ...
                    return (
                      <label
                        key={tf.val}
                        className={`relative flex items-center justify-center gap-2 overflow-hidden p-3 rounded-lg border text-sm font-semibold focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 focus-within:ring-offset-background ${
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Button variant="outline" onClick={resetExam} disabled={isSubmitting || isRegenerating} className="rounded-xl gap-2">
