...
| **E10** | frontend-engineer | teacher UI: service + wizard + gradebook + override + toggles | ✅ DONE — evolved on 2026-07-08 from the earlier split flow into a single intake card: title + deadline/settings + teacher note + source files + per-file hints up front, then a single `ساخت پیش‌نویس تمرین` action and persistent workflow/progress cards until `بازبینی و انتشار`; 2026-07-08 follow-up added a site-native **Jalali deadline picker** for both intake and post-extraction settings, replacing the browser-native Gregorian control. The old reference-ingest panel remains as `افزودن منبع تکمیلی`. |
