...
| `plans/ai-draft/` | POST | مشاور | `{prompt: str ≤2000}` → پیش‌نویس DRAFT WeeklyPlan (گام ۵)؛ `students/<int:pk>/plans/ai-draft/` |
| `plans/ai-draft/voice/` | POST | مشاور | multipart audio (≤۶۰ثانیه) → transcription → همان مسیر (گام ۶) |
...
### گام ۵ — برنامه‌ساز هوشمند (متن) · حجم: L — **تک‌استثنای ق۱′**
...
**فرانت:**
- `services/advisory-ai-planner.ts` + در تب «برنامه» بالای quick-add: کارت جمع‌شونده «برنامه‌ساز هوشمند» — textarea پرامپت + دکمۀ «تولید پیش‌نویس» (loading state) → پس از موفقیت، همان ادیتور پلنر با Draft باز می‌شود (مشاور ویرایش/انتشار می‌کند — مسیر موجود).
...
### گام ۶ — برنامه‌ساز صوتی · حجم: S (پس از گام ۵)

- همان endpoint با `voice=true` multipart: `transcribe_media_bytes` (موجود) → متن → مسیر گام ۵. سقف ۶۰ ثانیه/۵MB ⇒ 400 «فایل صوتی حداکثر ۶۰ ثانیه است.»
- فرانت: دکمۀ میکروفون کنار textarea (MediaRecorder، ضبط/توقف، پیش‌نمایش متن).
